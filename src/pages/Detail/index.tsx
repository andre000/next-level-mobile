...
import { View, StyleSheet, Linking, Image, Text, TouchableOpacity } from 'react-native';
...
const Detail = () => {
  ...
  useEffect(() => {
    api.get(`/points/${routeParams.point_id}`).then(({ data }) => {
      setPoint(data);
    });
  }, []);
  ...
  const handleComposeMail = () => {
    point && MailComposer.composeAsync({
  ...
  const handleWhatsApp = () => {
    point && Linking.openURL(`whatsapp://send?phone=${point.whatsapp}`);
  }
